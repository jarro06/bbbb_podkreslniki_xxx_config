...
def job_config = [
    job: [
        name: "bbbb_podkreslniki_xxx_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
ghe: [
        ...
        GitGroup: 'jarro06',
        RepoName: 'bbbb_podkreslniki_xxx',
        checks: [
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "jarro06", "bbbb_podkreslniki_xxx", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("bbbb_podkreslniki_xxx_Feature_Build");
